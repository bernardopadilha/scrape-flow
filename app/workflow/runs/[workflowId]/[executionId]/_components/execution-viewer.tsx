...
import { GetPhasesTotalCost } from '@/lib/helper/get-phases-total-cost'
...
export default function ExecutionViewer({
  ...
  const credtsConsumed = GetPhasesTotalCost(query.data?.phases || [])
      ...
      <aside className="w-[440px] min-w-[440px] max-w-[440px] border-r-2 border-separate flex flex-grow flex-col overflow-hidden">
        <div className="py-4 px-2">
          ...
          <ExecutionLabel
            icon={ClockIcon}
            label="Duration"
            value={duration || <Loader2Icon className="animate-spin size-5" />}
          />
          <ExecutionLabel
            icon={CoinsIcon}
            label="Credits consumed"
            value={<ReactCountUpWrapper value={credtsConsumed} />}
          />
          <Separator />

          <div className="flex justify-center items-center py-2 px-4">
            <div className="text-muted-foreground flex items-center gap-2">
              <WorkflowIcon className="size-5 stroke-muted-foreground/80" />
              <span className="font-semibold">Phases</span>
            </div>
          </div>
        </div>
        <Separator />
        <div className="overflow-auto h-hull px-2 py-4">
        ...
        {!isRunning && selectedPhase && phaseDetails.data && (
          <div className="flex flex-col py-4 container gap-4 overflow-auto">
            <div className="flex gap-2 items-center">
              <Badge variant={'outline'} className="gap-4">
                <div className="flex gap-1 items-center">
                  <CoinsIcon className="size-[18px] stroke-muted-foreground" />
                  Credits
                </div>
              ...
              <Badge variant={'outline'} className="gap-4">
                <div className="flex gap-1 items-center">
                  <ClockIcon className="size-[18px] stroke-muted-foreground" />
                  Duration
                </div>
                <span>
                  {DatesDurationString(
...
function ExecutionLabel({
  ...
  return (
    <div className="flex justify-between items-center py-2 px-4 text-sm">
      <div className="text-muted-foreground flex items-center gap-2">
        <Icon className="size-5 stroke-muted-foreground/80" />
        <span>{label}</span>
      </div>
      <div className="font-semibold capitalize flex gap-2 items-center">
        {value}
